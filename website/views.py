...
def send_inquiry_email(inquiry):
    """Helper function to send inquiry emails."""
    subject = f"New Inquiry from Website - {inquiry.get_service_display()}"
    message = f"""
Name: {inquiry.name}
Phone: {inquiry.phone}
Email: {inquiry.email}
...
def home(request):
    ...
    return render(
        request,
        "website/home.html",
        {"form": form, "menu_sections": menu_sections, "rooms": rooms},
    )
...
def room_detail(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            inquiry.service = "room"
            inquiry.save()
            send_inquiry_email(inquiry)
            messages.success(
                request, f"Booking inquiry for {room.name} sent successfully!"
            )
            return redirect("room_detail", slug=slug)
    else:
        # Pre-fill message with room name
        initial_data = {
            "service": "room",
            "message": f"I am interested in booking the {room.name}.",
        }
        form = InquiryForm(initial=initial_data)

    return render(request, "website/room_detail.html", {"room": room, "form": form})
